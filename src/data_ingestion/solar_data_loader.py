...
OUTPUT_DIR = "data/raw/solar"
FINAL_FILE = "data/raw/solar/solar_raw.csv"
...
def fetch_solar_data():
    ...
    parameters = [
        "CLOUD_AMT", "AOD_55", "PW", "QV2M", "T2M",
        "RH2M", "SZA", "WS50M", "PS", "TOA_SW_DWN", "ALLSKY_SFC_SW_DWN"
    ]
    ...
    for state in states:
        print(f"Fetching Solar Data for {state['name']}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if idx != -1:
                    ...
                    filename = os.path.join(OUTPUT_DIR, f"{state['name']}_solar.csv")
...
def merge_solar_data():
    ...
    if not files:
        print("No solar files found!")
        return

    df = pd.concat([pd.read_csv(f) for f in files], ignore_index=True)
    df.to_csv(FINAL_FILE, index=False)

    print(f"Solar master dataset saved at: {FINAL_FILE}")
